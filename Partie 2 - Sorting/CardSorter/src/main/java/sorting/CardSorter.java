...
public class CardSorter {
    ...
    public static void sort(LinkedListImpl list) {
        ...
        for (int iter = 0; iter < list.getSize() - 1; iter++) {
            ...
            /*
            faire une sorte de BubbleSort
            faire une seconde boucle imbriquée (complexité max en O(n^2) donc c'est autorisé)

             */

            for (int j = 0; j < list.getSize() - iter - 1; j++){
                if (list.getFirst() > list.getSecond()){ //besoin de swap car pas dans le bon ordre
                    list.swap();
                }
                list.pop(); //on pop() pour passer aux 2 noeuds suivant et avancer dans la liste
            }

            for (int k = 0; k <= iter;k++){ //troisième boucle pour remettre les éléments de devant à l'arrière
                list.pop();
            }
