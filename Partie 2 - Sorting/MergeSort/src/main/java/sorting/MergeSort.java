...
public class MergeSort {
    ...
    private static void sort(Comparable[] a, Comparable[] aux, int lo, int hi) {
        // TODO
        if (hi <= lo) return; //si hi plus petit que lo, stop
        int mid = lo + (hi-lo) / 2;
        sort(a,aux,lo,mid); //sort de la partie gauche
        sort(a,aux,mid+1,hi); //sort de la partie droite
        merge(a,aux,lo,mid,hi); //merge des 2
    }
    ...
    public static void sort(Comparable[] a) {
        // TODO
        Comparable[] aux = new Comparable[a.length]; //nouvel array aux de même taille que a
        sort(a,aux,0,a.length-1); //appel à l'autre fonction de sort
    }
}
