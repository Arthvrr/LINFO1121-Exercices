...
public class MinPQLinked<Key> {
    ...
    public Key delMin() {
        ...
        /*
        retire et retourne la plus petite clé de la PriorityQueue
        la plus petite se trouve à la racine
        1. récupérer la valeur de la clé à la racine
        2. retrouver le dernier noeud inséré de la PriorityQueue --> le plus à droite sur le dernier niveau
        3. échanger la root et ce dernier noeud
        4. supprimer la root tout en bas
        5. restaurer l'invariant de la Heap
         */
        if (isEmpty()) throw new NoSuchElementException();
        Key rootVal = min();

        if (root.left == null & root.right == null){ //si on a un seul noeud
            root = null; //on met la root à null
            return rootVal;
        }

        Node lastNode = root; //on trouve le dernier noeud inséré (on démarre à la root)
        String binaire = Integer.toBinaryString(root.size); //conversion en binaire de l'entier (13 --> "1101")
        for (int i = 1; i < binaire.length(); i++){
            char bit = binaire.charAt(i);
            if (bit == '0'){ //si le bit vaut 0, on va à gauche
                lastNode = lastNode.left;
            } else lastNode = lastNode.right; //si le bit vaut 1, on va à droite
        }
        exch(root,lastNode); //on échange la racine et le dernier noeud

        if (lastNode.parent.left == lastNode) lastNode.parent.left = null; //on supprime le lien du parent du lastNode
        else lastNode.parent.right = null;

        Node tmp = lastNode.parent; //on met à jour les tailles des parents
        while (tmp != null) {
            tmp.size--;
            tmp = tmp.parent;
        }
        sink(root); //on restaure l'invariant de la Heap
        return rootVal;
    }
