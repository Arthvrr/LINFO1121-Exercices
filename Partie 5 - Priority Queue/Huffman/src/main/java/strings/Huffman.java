...
public class Huffman {
    ...
    public static HuffmanNode buildTrie(int[] freq) {
        ...
        //on ajoute tout les éléments de freq à la PriorityQueue
        for (int i = 0; i < freq.length; i++){
            pq.add(new HuffmanNode(i,freq[i],null,null));
        }

        while (pq.size() > 1){ //on fait tant qu'il reste au moins 2 noeuds pour s'arrêter à la racine du trie et pouvoir le retourner à la fin
            HuffmanNode left = pq.poll();
            HuffmanNode right = pq.poll();
            HuffmanNode parent = new HuffmanNode(-1, left.getFrequency()+ right.getFrequency(),left,right); //ici la valeur du noeud (-1) n'a pas d'importance
            pq.add(parent);
        }
        return pq.poll();
    }
...
class HuffmanNode implements Comparable<HuffmanNode> {
    ...
    @Override
    public int compareTo(HuffmanNode node) {return this.freq - node.freq; } //c'est la freq qui détermine la priorité
