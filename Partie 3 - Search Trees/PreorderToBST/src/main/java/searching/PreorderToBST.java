...
public class PreorderToBST {
    ...
    */
    ...
    public Node preorderRead(int [] preOrderInput, int i, int min, int max) {
        ...
        if (preOrderInput.length <= i){ //si i dépasse la longueur du tableau, on stop
            return null;
        }

        if (max <= min){ //si min plus grand que max, erreur
            return null;
        }
        if (preOrderInput[i] < min || preOrderInput[i] > max){ //si élément itéré pas dans la fenêtre [min,max]
            return null;

        } else { //élément itéré dans la fenêtre, on est bon

            /*
            construction du noeud left
            calcul de la size de la partie gauche du tree
            construction du noeud right en prenant en compte la size de la partie droite du tree
            retourner le noeud racine root(left,right)
             */

            Node left = preorderRead(preOrderInput,i+1,min,preOrderInput[i]); //appel récursif avec i+1 et fenêtre [min,preOrderInput[i]]
            int leftSize = left != null ? left.size : 0; //calcul de la size de la partie gauche pour avoir le bon index
            Node right = preorderRead(preOrderInput,i+1+leftSize,preOrderInput[i],max); //appel récursif avec i+1+leftSize et fenêtre [preOrderInput[i],max]

            return new Node(left,right,preOrderInput[i]); //on retourne le nouveau noeud composé des noeuds gauche et droite
        }
    }
