...
public class ArrayBSTDelete<Key extends Comparable<Key>, Value> {
    ...
    public ArrayList<Boolean> deleted; //nouvelle ArrayList de booléens si oui ou non un noeud a été supprimé
    ...
    private void addNode(Key key, Value val) {
        ...
        deleted.add(false); //noeud non-supprimé, on ajoute false
    }
        ...
        if (!values.isEmpty()) {
            ...
            do {
                ...
                if (cmp == 0) {
                    ...
                    values.set(i,val);
                    if (deleted.get(i) == true){ //si la clé avait été supprimée, on la remet en false et on retourne true
                        deleted.set(i,false);
                        return true;
                    }
                    return false;
    ...
    public Value get(Key key) {
        ...
        if (deleted.get(i)) return null; //on ignore les noeuds marqués comme supprimés
        return values.get(i);
    }
    ...
    public boolean delete(Key key) {
        // TODO
        if (keys.isEmpty()) return false; //arbre vide, on a rien supprimé
        int index = getNodeIndex(key); //on récupère l'index de la clé
        if (index == NONE) return false; //getNodeIndex() retourne NONE si la clé n'existe pas
        if (deleted.get(index)) return false; //clé déjà supprimée auparavant
        if (!deleted.get(index)) deleted.set(index,true); //on passe de false à true
        return true; //clé supprimée comme prévu
    }
