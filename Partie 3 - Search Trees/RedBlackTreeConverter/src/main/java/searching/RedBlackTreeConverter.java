...
public class RedBlackTreeConverter {
    ...
    public static<Key extends Comparable<Key>> RBNode<Key> convert(TwoThreeNode<Key> twoThreeNode) {
        ...
        if (twoThreeNode == null){ //si root null on retourne null
            return null;
        }

        //identifier si on a un 2-node ou un 3-node
        if (twoThreeNode.is2node()){ //2-node --> noeud noir dans RBTree

            RBNode<Key> newNode = new RBNode<>(twoThreeNode.leftKey, twoThreeNode.leftValue, Color.Black,1); //on crée le nouveau noeud
            newNode.leftChild = convert(twoThreeNode.leftChild); //appels récursifs vers les enfants de twoThreeNode
            newNode.rightChild = convert(twoThreeNode.centerChild);
            newNode.size = sizeEvenIfNull(newNode.leftChild) + 1 + sizeEvenIfNull(newNode.rightChild); // appels récursifs pour compter la size du RBTree
            return newNode;

        } else { //3-node --> noeud rouge dans RBTree
            /*
            si c'est un 3-node dans le 2-3 tree :
            - clé de gauche devient noeud rouge
            - clé de droite devient noeud noir
            - noeud rouge est l'enfant gauche du noeud noir
             */

            RBNode<Key> redNode = new RBNode<>(twoThreeNode.leftKey, twoThreeNode.leftValue, Color.Red,1); //on crée les 2 nouveaux noeuds
            ...
            blackNode.leftChild = redNode; //noeud rouge est l'enfant gauche du noeud noir
            blackNode.rightChild = convert(twoThreeNode.rightChild);
            redNode.size = sizeEvenIfNull(redNode.leftChild) + 1 + sizeEvenIfNull(redNode.rightChild); //appels récursifs pour compter la size du RBTree
            blackNode.size = sizeEvenIfNull(blackNode.leftChild) + 1 + sizeEvenIfNull(blackNode.rightChild);
            return blackNode;
        }
    }
