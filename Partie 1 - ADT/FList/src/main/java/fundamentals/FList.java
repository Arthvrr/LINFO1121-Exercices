...
public abstract class FList<A> implements Iterable<A> {
    ...
    public final int length() {
        // TODO
        return 1 + tail().length(); //récursif, 1 + la taille de la queue
    }
    ...
    public final <B> FList<B> map(Function<A,B> f) {
        // TODO
        if (isEmpty()) return nil(); //si vide on retourne nil
    ...
    public final FList<A> filter(Predicate<A> f) {
        // TODO
        if (isEmpty()) return nil(); //si vide on retourne nil
        if (f.test(head())) return tail().filter(f).cons(head()); //si respecte le filtre on print avec cons()
    ...
    public Iterator<A> iterator() {
        return new Iterator<A>() {
            ...
            public A next() {
                // TODO
                A next = current.head();
                current = current.tail(); //on passe à l'élément suivant
                return next; //on retourne la head actuelle
            }
    ...
    private static final class Nil<A> extends FList<A> {
        ...
        public A head() {
            // TODO
            throw new IllegalArgumentException(); //Nil, n'a pas de head ni tail
        }
