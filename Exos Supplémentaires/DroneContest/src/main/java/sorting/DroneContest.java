...
public class DroneContest {
    ...
    public static LinkedList<HeightChange> findHighest(Drone[] participants) {
        ...
        ArrayList<Event> events = new ArrayList<>(); //on construit la liste sur lequel on va itérer pour construire notre liste result
        for (Drone drone : participants){
            events.add(new Event(drone.start,true, drone.height));
            events.add(new Event(drone.end,false, drone.height));

        }

        events.sort(null); //fonctionne car Comparable implémenté dans la classe Event
        PriorityQueue<Integer> maxHeap = new PriorityQueue<>(Collections.reverseOrder());
        maxHeap.add(0); //sol au départ

        int prevMax = 0;
        result.add(new HeightChange(0,0)); //ajout de 0,0 au départ

        for (Event event : events){ //on parcoure tout les éléments de la liste events
            if (event.isStart){
                maxHeap.add(event.height); //on ajoute la hauteur à la Heap si c'est un start
            } else maxHeap.remove(event.height); //on enlève cette hauteur de la heap si c'est un end

            int currentMax = maxHeap.peek(); //hauteur maximale actuelle

            if (currentMax != prevMax){ //si hauteur maximale différente que prevMax, on ajoute à result
                result.add(new HeightChange(event.time,currentMax));
                prevMax = currentMax;
            }
        }
        return result;
    ...
    private static class Event implements Comparable<Event>{
        ...
        public Event(int time, boolean isStart, int height){
            this.time = time;
        ...
        @Override
        public int compareTo(Event other){ //triée en fonction du temps, puis si start par rapport à end
            if (this.time != other.time) {
                return Integer.compare(this.time, other.time); // temps croissant
            }
            if (this.isStart != other.isStart){
                if (this.isStart){
                    return -1; //start passent avant les end
                } else return 1;
            }
            return 0;
        }
        ...
        public String toString(){
            if (this.isStart) return this.time + " & start & " + this.height;
            else return this.time + " & end & " + this.height;
        }
    }
}
