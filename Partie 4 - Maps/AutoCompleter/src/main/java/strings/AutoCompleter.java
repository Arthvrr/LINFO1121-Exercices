...
public class AutoCompleter {
    ...
    public String complete(String prefix) {
        ...
        //on descend dans le trie pour commencer exactement à la fin du préfixe
        Node node = root;
        for (int i = 0; i < prefix.length(); i++){
            node = node.next[charToIndex(prefix.charAt(i))]; //on passe au noeud/caractère suivant du trie/préfixe
            if (node == null) return null;
        }

        //queue pour stocker les noeuds et mots correspondants
        Queue<NodeWord> queue = new LinkedList<>();
        queue.add(new NodeWord(node,prefix));

        while (!queue.isEmpty()) {
            NodeWord current = queue.poll();
            if (current.node.isKey) return current.word; //marque la fin d'un nom complet, on retourne tout de suite (mot le + court lexicographiquement)

            for (int i = 0; i < R; i++) {
                if (current.node.next[i] != null) { //on regarde tout les enfants du noeud courant
                    queue.add(new NodeWord(current.node.next[i], current.word + (char)(i + 'a')));
                }
            }
        }
        return null; //rien trouvé, on retourne null
    }

    private static class NodeWord {
        Node node;
        String word;
        NodeWord(Node n, String w) {
            node = n;
            word = w;
        }
    }
