...
public class Monkeys {
    ...
    private static Hashtable<String,Monkey> monkeysList;
     ...
     */
    public static long evaluateRoot(List<Monkey> input) throws Exception {
        //TODO
        monkeysList = new Hashtable<>();
        for (Monkey currentMonkey : input){ //on ajoute toutes les paires nom de singe-singe au Hashtable
            monkeysList.put(currentMonkey.name,currentMonkey);
        }
        return evaluate(monkeysList.get("root")); //on démarre la recherche à la racine
    }

    /*
    fonction récursive permettant de trouver la réponse finale
     */
    public static long evaluate(Monkey monkey) throws Exception {

        if (monkey instanceof YellingMonkey){ //si YellingMonkey, on retourne juste son nombre
            return ((YellingMonkey) monkey).number;

        } else { //si OperationMonkey, on va procéder récursivement vers ses Singes gauches et droits

            OperationMonkey opMonkey = (OperationMonkey) monkey; //cast vers OperationMonkey
            String leftMonkey = opMonkey.leftMonkey;
            char operation = opMonkey.op;
            String rightMonkey = opMonkey.rightMonkey;

            long evalueLeft = evaluate(monkeysList.get(leftMonkey)); //appels récursifs des singes gauches et droits
            long evalueRight = evaluate(monkeysList.get(rightMonkey));

            if (operation == '+'){
                return evalueLeft + evalueRight;
            } else if (operation == '-') {
                return evalueLeft - evalueRight;
            } else if (operation == '*'){
                return evalueLeft * evalueRight;
                ...
                return evalueLeft / evalueRight;
            } else throw new Exception("Illegal Operation !");
        }
    }
